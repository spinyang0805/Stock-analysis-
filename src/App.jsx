    ...
    async function load() {
      ...
      try {
        ...
        if (apiReady) {
          ...
          nextAnalysis = demoAnalysis(symbol);
        }
        ...
        candleRef.current?.setData(kline);
        setAnalysis(demoAnalysis(symbol));
      ...
      <header style={{ padding: "22px 28px", borderBottom: "1px solid #1e293b", background: "#0f172a" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
          <div>
            ...
            <h1 style={{ margin: "6px 0 0", fontSize: 28 }}>交易決策系統</h1>
          </div>
        ...
        <div style={{ marginTop: 18, display: "flex", gap: 10, flexWrap: "wrap" }}>
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && setSymbol(input.trim() || "2330")}
            placeholder="輸入股票代號，例如 2330"
            style={{ padding: "12px 14px", borderRadius: 10, border: "1px solid #334155", background: "#020617", color: "white", minWidth: 260 }}
          />
          <button
            onClick={() => setSymbol(input.trim() || "2330")}
            style={{ padding: "12px 18px", borderRadius: 10, border: 0, background: "#2563eb", color: "white", fontWeight: 700 }}
          >
            查詢分析
          </button>
        </div>
      ...
      <main style={{ padding: 24, display: "grid", gridTemplateColumns: "minmax(0, 2fr) minmax(320px, 0.9fr)", gap: 18 }}>
        <section style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 18, padding: 18 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <h2 style={{ margin: 0 }}>{symbol} K 線決策圖</h2>
            <span style={{ color: "#94a3b8" }}>可拖曳 / 縮放</span>
          </div>
        ...
        <aside style={{ display: "grid", gap: 14 }}>
          <section style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 18, padding: 18 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 8 }}>
              <div style={{ fontSize: 44, fontWeight: 800, color: scoreColor(analysis?.score ?? 0) }}>{analysis?.score ?? "--"}</div>
              <div>
                <div style={{ fontSize: 22, fontWeight: 700 }}>{analysis?.trend ?? "分析中"}</div>
                <div style={{ color: "#94a3b8" }}>{analysis?.rating ?? ""}</div>
              </div>
            </div>
            <div style={{ height: 10, borderRadius: 999, background: "#1e293b", overflow: "hidden", marginTop: 16 }}>
              <div style={{ width: `${Math.min(100, Math.max(0, ((analysis?.score ?? 0) + 60) / 120 * 100))}%`, height: "100%", background: scoreColor(analysis?.score ?? 0) }} />
            </div>
          </section>

          <section style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 18, padding: 18 }}>
            <h3 style={{ marginTop: 0 }}>決策摘要</h3>
            <p style={{ lineHeight: 1.7, color: "#cbd5e1" }}>{analysis?.summary ?? "正在建立分析..."}</p>
          </section>
        </aside>
        ...
        <section style={{ gridColumn: "1 / -1", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 18, padding: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12 }}>
            {(analysis?.signals || []).map((s, i) => (
              <div key={i} style={{ background: signalColor(s.score), border: "1px solid rgba(255,255,255,.08)", borderRadius: 14, padding: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                  <b>{s.title}</b>
                  <span>{s.score > 0 ? "+" : ""}{s.score}</span>
                </div>
                <p style={{ color: "#e2e8f0", lineHeight: 1.6, marginBottom: 0 }}>{s.message}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {analysis?.missing_data?.length > 0 && (
          <section style={{ gridColumn: "1 / -1", background: "#422006", border: "1px solid #92400e", borderRadius: 18, padding: 18 }}>
            <h3 style={{ marginTop: 0 }}>資料限制 / 待串接</h3>
            <ul>
              {analysis.missing_data.map((m, i) => <li key={i}>{m}</li>)}
            </ul>
          </section>
        )}
      </main>
