...
def get_stock_universe(market_filter: str = "all") -> list:
    ...
    if len(db_products) >= 200:
        ...
        if market_filter == "twse":
            return [p for p in db_products if p.get("market") in ("上市", "TWSE")]
        if market_filter == "tpex":
            return [p for p in db_products if p.get("market") in ("上櫃", "TPEx", "OTC")]
        return db_products
...
def main():
    ...
    print(f"  {'代號':<6} {'名稱':<10} {'市場':<5} {'新增':>5}  狀態")
    print("  " + "-" * 52)
    ...
    for i, item in enumerate(stocks, start=1):
        ...
        try:
            ...
            if written > 0:
                ok_count += 1
                print(f"  {code:<6} {name:<10} {market_key:<5} {written:>5}筆  ✅ ({elapsed:.1f}s)")
            else:
                err_count += 1
                errmsg = (errors[0][:45] if errors else "無資料或 API 無回應")
                print(f"  {code:<6} {name:<10} {market_key:<5} {'0':>5}筆  ⚠️  {errmsg}")
        except Exception as e:
            err_count += 1
            print(f"  {code:<6} {name:<10} {market_key:<5} {'?':>5}筆  ❌ {str(e)[:45]}")
